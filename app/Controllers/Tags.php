...
class Tags extends BaseController
{
    ...
    public function index()//: string
    {
        ...
        $tagsdata           = $TagsModel->like('title', $input['tag'])->first();
        ...
        $data                   = $this->data;
        $data['title']          = "Artikel Terkait ".$input['tag'];
        $data['description']    = "Artikel Terkait ".$input['tag'];
        $data['result']         = array_slice($result, ($page*20)-20, $page*20);
        $data['pager_links']    = $pager->makeLinks($page, $perPage, $total, 'uikit_full');
        $data['caturi']         = 'tag?tag='.$input['tag'];
        $data['cattitle']       = $input['tag'];
