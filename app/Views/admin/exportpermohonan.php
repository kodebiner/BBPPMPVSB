...
<head>
    ...
    <title>Daftar Permohon Informasi</title>
    <style>
        html {
            font-size: 8pt
        }
...
<body>
	...
	header("Content-Disposition: attachment; filename=Daftar-Permohonan-Informasi-".$bulans."-".$tahun.".xls");
	?>

    <div style="text-align: center; font-size: 30px; font-weight: 800;">Laporan Permohon Informasi</div>
    ...
    <table>
        ...
        <thead>
            <tr>
                ...
                <th style="margin: 15px;">Alamat</th>
                <th style="margin: 15px;">Pekerjaan</th>
        ...
        <tbody>
            <?php foreach ($permohonan as $mohon) { ?>
                <tr>
                    <td style="margin: 15px;"><?= $mohon['created_at'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['name'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['email'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['phone'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['address'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['jobs'] ?></td>
                    <td style="margin: 15px;"><?= $mohon['note'] ?></td>
                    <td style="margin: 15px;">
                        <?php if ($mohon['userid'] != null) {
